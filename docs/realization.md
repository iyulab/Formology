...
- 부분 적합: SNS, 이커머스, 교육
- 부적합: 순수 알고리즘, 실시간 제어 시스템, 게임, 원시 스트림 처리

> **센서·IoT는 부적합 목록에 없습니다.** 원신호는 시계열 저장소로 가고, 교대 종료·LOT 완료·임계 초과 같은 **확정 시점에 생성되는 일지와 기록지**가 Formology의 대상입니다. 제조 MES가 대표 적합 사례인 이유이기도 합니다 — [방법론 「신호는 어디에 있는가」](methodology.md#신호는-어디에-있는가).
