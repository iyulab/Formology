...
6. [사례 연구](#6-사례-연구) — 제조 MES (ROI 203%)
7. [FAQ](#7-faq)
8. [용어 사전](#8-용어-사전)
...
소요 시간: 워크숍 2시간 + DDL 4시간 = 6시간 (전통 ERD 방식: 2-4주)
```
...
**워크숍 성과**: 22개 서식 도출, 7개 MVP 선정, 4개 주요 양식 구조 스케치, ERD 초안 자동 도출. **소요 시간: 2시간 15분**.
...
**효과**: 코드 재사용성 80% 향상, 새 문서 추가 시간 70% 단축, 일관된 UX.
...
3. **즉시 검증**: "이 문서 목록 맞아요?" → "네!" (100% 합의). 1개월 후 "화면이 양식이랑 똑같네요."
4. **자연스러운 진화**: 초기 22개 → 1개월 +2개 → 3개월 +3개. 강요 없이 필요 시 추가.

### 6.7 교훈

**잘된 점**: 워크숍 충실 이행 (2시간→3개월 단축), MVP 우선순위 엄수 (7개만), 패턴 내부화 (현업 비노출), 즉시 피드백 반영 (모바일 2주 해결)

**아쉬운 점**: 모바일 초기 미고려 (현장은 태블릿 선호), 바코드 늦은 도입, 통계 우선순위 낮춤 (경영진에게 매우 중요)
...
| 요구사항 정의 | 3주 | 2일 | 90% 감소 |
| 현업 이해도 | 40% | 95% | 137% 향상 |
| 요구사항 변경 | 주 3회 | 주 0.3회 | 90% 감소 |
| 데이터 모델 정확도 | 70% | 95% | 36% 향상 |
