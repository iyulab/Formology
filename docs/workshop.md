...
**이미 돌아가는 시스템이 있는 경우**도 이 절차를 그대로 쓰지 않습니다. 단계를 하나 앞에 붙입니다 — 기존 화면과 데이터를 놓고 **"이건 원래 무슨 서류였습니까?"**를 먼저 묻습니다(역변환). 역변환이 되는 화면은 서식으로 되돌리고, 안 되는 화면은 그대로 둡니다. 신규 기능부터 Formology 방식으로 만들고 기존은 점진적으로 옮깁니다.

다부서 통합의 경우 **도메인별로 워크숍을 반복한 뒤 통합 세션을 따로** 여는 방식이 됩니다. 다만 **이 확장 절차는 아직 검증되지 않았습니다.** 아래 본문과 실측 사례([사례 연구](casestudy.md))는 단일 도메인 절차입니다.
...
- 기록자 1명 — 빠른 타이핑, 구조화 능력, 질문 능력. 스케치하지 않고 넘어간 2차 문서를 [역량질문 목록](#37-역량질문-목록-2차-문서-목록)으로 따로 모으는 것도 기록자 몫입니다
...
| 3:05–3:20 | Step 9. 확인 및 정리 | 체크리스트, 역량질문 목록 확정, 우선순위 최종 확인 | 최종 산출물 |
...
> **주의 — "누구에게" 중요한가.** 매트릭스를 채우는 사람이 곧 중요도 판정의 표본입니다. 현장 실무자만 있는 자리에서는 경영진이 보는 서식이 반드시 아래로 내려갑니다. 실제로 그렇게 되어 손해를 본 사례가 있습니다([사례 연구 7](casestudy.md#7-실패한-것--그리고-그것이-절차를-어떻게-바꿨는가)). 배치할 때마다 **"이거 주로 누가 봐요?"**를 함께 물으십시오.
...
> **2차 서식은 고르지 마십시오.** 현황표·집계표·이력록처럼 **다른 문서를 모아 보는 것**들입니다([방법론 「분류의 세 축」](methodology.md#분류의-세-축)). 재고현황표를 스케치하려 하면 시간이 사라지고, 그럴 필요도 없습니다. 입고확인서와 출고확인서가 있으면 재고현황표는 따라 나옵니다.
>
> 판정은 질문 하나로 합니다 — *"이건 누가 채웁니까, 아니면 저절로 채워집니까?"* 저절로 채워지는 것이면 한 줄만 적고 넘어갑니다.
>
> **그 한 줄은 버리지 마십시오.** "재고현황표 — 입고확인서와 출고확인서를 모아 본다"는 곧 *"이 시스템은 재고 현황을 낼 수 있는가?"* 라는 질문이고, 도출된 구조는 나중에 정확히 이 질문들로 검증됩니다. 이것이 [역량질문](glossary.md#역량질문-competency-question)입니다 — 스케치하지 않은 2차 문서 하나가 질문 하나입니다. 기록자는 별도 목록([산출물 3.7](#37-역량질문-목록-2차-문서-목록))에 모읍니다.
...
| **기재 주체** | **"이 칸은 누가 채우세요?"** (신청인 / 담당자 / 부서장) |
| **바인딩** | **"이 칸은 저쪽이 바뀌면 같이 바뀌어야 하나요, 그대로 남아야 하나요?"** |

**그리고 서식 한 장에 한 번씩만 묻는 것 셋** — 칸마다 묻지 않습니다:
...
> 이 셋은 [실제 프로젝트에서 놓쳐 손해를 본 뒤 추가된 것](casestudy.md#7-실패한-것--그리고-그것이-절차를-어떻게-바꿨는가)입니다. 빼지 마십시오. 셋 다 묻는 데 1분이 걸리고, 안 물으면 각각 2주가 듭니다.
>
> **그리고 서식마다 한 번 더**(시범) — *"이 서식, 왜 씁니까 — 법으로 정해진 겁니까, 계약 때문입니까, 내부 규정입니까, 그냥 늘 해오던 겁니까?"* 답이 그 서식의 [존재 근거(mandate)](glossary.md#존재-근거-mandate--시범)입니다. "늘 해오던 것"이라는 답은 틀린 답이 아니라 **걷어낼 후보 표시**입니다 — 근거를 못 대는 서식이 디지털화해서 굳힐 가치가 가장 낮습니다.
>
> **기재 주체**와 **바인딩**은 별도 항목이 아니라 **칸을 그리는 중에 붙는 꼬리**입니다. "이건 부서장이 사인하는 칸이죠?", "제품명은 마스터에서 고르시죠 — 나중에 바뀌면 이 서류도 바뀌어야 하나요?" 순증 시간이 사실상 없고, 답이 갈리는 칸에만 표시를 남기면 됩니다. 각각 [방법론 「기재 주체」](methodology.md#기재-주체--누가-이-칸을-채우는가)·[「필드 단위 세분」](methodology.md#필드-단위-세분--섹션이-기본값을-정하고-필드가-예외를-선언한다).
...
- [ ] 데이터 관계가 명확한가?
- [ ] 역량질문 목록(넘어간 2차 문서 전부)이 모였고, 각각 어느 1차 문서들을 모아 보는지 적혀 있는가?
...
### 3.7 역량질문 목록 (2차 문서 목록)

Step 7에서 스케치하지 않고 한 줄만 적고 넘어간 2차 문서들입니다. 각 줄이 도출된 구조에 던지는 질문 하나이고, 이 목록이 **"도출 결과가 좋은가"를 판정하는 유일한 산출물**입니다 — 양식 스케치와 ERD 초안은 구조를 말하지만, 그 구조가 현업이 보고 싶은 것을 낼 수 있는지는 이 목록만 말합니다.

| 역량질문 (2차 문서) | 모아 보는 1차 문서 | 도출 후 답할 수 있는가 |
|---|---|---|
| 재고현황표 — 품목별 현재고를 볼 수 있는가 | 입고확인서, 출고확인서 | ☐ |
| 월별생산현황표 — 설비별·월별 생산량을 볼 수 있는가 | 작업일지 | ☐ |
| 불량이력록 — 제품별 불량 발생 이력을 시간순으로 볼 수 있는가 | 불량기록지 | ☐ |

세 번째 열은 워크숍에서 채우지 않습니다. 도출([도출 원리 §1.9](derivation.md#19-품질-체크리스트))이 끝난 뒤 하나씩 확인하며, **답하지 못하는 질문 하나가 곧 빠진 엔티티나 관계 하나**입니다 — 그것이 이 목록을 보존하는 이유입니다.
...
> 위 이력은 [제조 MES 사례](casestudy.md)의 실제 경과입니다. 4개월 동안 22개에서 27개로 늘었고, **전부 기존 목록에 얹히는 증분이었습니다.** 설계를 뒤집는 변경은 없었습니다 — 서식이 다섯 개 늘어난 것과 데이터 모델을 다시 그리는 것은 다른 일입니다.
...
플라스틱 사출 공장 사례([사례 연구](casestudy.md))에서 워크숍 산출물이 어떻게 쓰였는가:
